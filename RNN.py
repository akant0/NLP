# coding: utf-8
# NLU assignment creating a recurrent neural network for language modeling 
import sys
import time
import numpy as np
...
class RNN(object):
    ...
    def predict(self, x):
        ...
        for t in range(len(x)):
            ...
            vx = np.dot(self.V, X)
            s[t] = sigmoid( vx + np.dot(self.U,s[t-1]))
    ...
    def acc_deltas(self, x, d, y, s):
        ...
        for t in reversed(range(len(x))):
            currentD = make_onehot(d[t], self.vocab_size)   # one hot for current desired
            currentY = y[t]                                 # one hot for current y, pred. output
            diffDY = currentD - currentY                    # diff of current desired and current y
            #dSoft = np.ones((1, self.vocab_size))          # deriv. of softmax, don't need
            dout = diffDY #* dSoft

            w = np.outer(dout, s[t])                        # outer dot prod d_out and current s
            self.deltaW += w                                # add to delta W

            currentX = make_onehot(x[t], self.vocab_size)   # one hot of current input
            #ones = np.ones((1, len(s[t])))                 # ones vector- don't need
            fDer = s[t] * (1 - s[t])                        # deriv. of sigmoid, f'
            din = np.dot(self.W.T, dout) * fDer             # delta_in

            v = np.outer(din, currentX)                     # outer prod d_in and current x input one hot
            self.deltaV += v                                # add to delta V

            u = np.outer(din, s[t-1])
            self.deltaU +=u
    ...
    def acc_deltas_np(self, x, d, y, s):
        ...
        t = len(x) - 1
        currentD = make_onehot(d[0], self.out_vocab_size)   # one hot for current desired
        currentY = y[t]                                     # one hot for current y, pred. output
        diffDY = currentD - currentY                        # diff of current desired and current y
        # dSoft = np.ones((1, self.vocab_size))             # der. of softmax, don't need

        dout = diffDY# * dSoft                              # d_out
        w = np.outer(dout, s[t])                            # outer dot prod d_out and current s
        self.deltaW += w                                    # add to delta W
        # ones = np.ones((1, len(s[t])))                    # ones vector, not needed
        fDer = s[t] * (1 - s[t])                            # deriv of sigmoid, f'
        din = np.dot(self.W.T, dout) * fDer                 # delta_in

        currentX = make_onehot(x[t], self.vocab_size)       # one hot of current x
        v = np.outer(din, currentX)                         # outer prod d_in and current x input one hot
        self.deltaV += v                                    # add to delta V

        u = np.outer(din, s[t - 1])                         # outer prod. of d_in and prev. s
        self.deltaU += u                                    # add to delta U
    ...
    # print("time {0}".format(t))
            currentD = make_onehot(d[t], self.out_vocab_size)   # one hot for current desired
            currentY = y[t]                                     # one hot for current y, pred. output
            diffDY = currentD - currentY                        # diff of current desired and current y
            # dSoft = np.ones((1, self.vocab_size))             # der. of softmax, not needed
            dout = diffDY  # * dSoft                            # d_out
            w = np.outer(dout, s[t])                            # outer dot prod d_out and current s
            self.deltaW += w                                    # add to delta W
            # ones = np.ones((1, len(s[t])))                    # ones vector, not needed
            fDer = s[t] * (1 - s[t])                            # deriv of sigmoid, f'
            din = np.dot(self.W.T, dout) * fDer                 # delta_in
            for step in range(t - steps, t + 1)[::-1]:
                if step >= 0:
                    # print("Backprop at time {0} {1}".format(t,step))
                    currentX = make_onehot(x[step], self.vocab_size)    # one hot of current secondary time's x
                    v = np.outer(din, currentX)                         # outer product of d_in and current x
                    self.deltaV += v                                    # add to delta V
                    u = np.outer(din, s[step - 1])                      # outer product of d_in and prev s
                    self.deltaU += u                                    # add to delta U
                    fDer = s[step - 1] * (1 - s[step - 1])              # deriv of sigmoid, f'
                    din = np.dot(self.U.T, din) * fDer                  # dot prod. of U tr. and d_in, * deriv of sigmoid
    ...
    def acc_deltas_bptt_np(self, x, d, y, s, steps):
        ...
        t = len(x)
        currentD = make_onehot(d[0], self.out_vocab_size)           # one hot for current desired
        currentY = y[t-1]                                           # one hot for current y, pred. output
        diffDY = currentD - currentY                                # diff of current desired and current y
        # dSoft = np.ones((1, self.vocab_size))                     # der. of softmax, not needed
        dout = diffDY  # * dSoft                                    # d_out
        w = np.outer(dout, s[t-1])                                  # outer dot prod d_out and current s
        self.deltaW += w                                            # add to delta W
        # ones = np.ones((1, len(s[t])))                            # ones vector, not needed
        fDer = s[t-1] * (1 - s[t-1])                                # deriv of sigmoid, f'
        din = np.dot(self.W.T, dout) * fDer                         # delta_in

        for step in range(t - steps - 1, t)[::-1]:
            if step >= 0:
                # print("Backprop at time {0} {1}".format(t,step))
                currentX = make_onehot(x[step], self.vocab_size)    # one hot of current secondary time's x
                self.deltaV += np.outer(din, currentX)              # add to delta V
                self.deltaU += np.outer(din, s[step - 1])           # outer product of d_in and prev s
                fDer = s[step - 1] * (1 - s[step - 1])              # add to delta U
                din = np.dot(self.U.T, din) * fDer                  # dot prod. of U tr. and d_in, * deriv of sigmoid
    ...
    def compute_loss(self, x, d):
        ...
        Y = self.predict(x)[0]                          # bc self.predict returns y, s
        D = np.zeros(shape=(len(x), self.vocab_size))   # zeros, len x to vocab size
        for t in range(len(d)):                         # for each spot in the new matrix
             D[t] = make_onehot(d[t], self.vocab_size)  # fill it with one hot of deisred
        loss = - np.ndarray.sum(D * np.log(Y))          # loss is neg sum of desired matrix and log of predicted
    ...
    def compute_loss_np(self, x, d):
        ...
        Y = self.predict(x)[0]
        # loss is neg sum of a matrix with one hots of the desired * log of final Y
        loss = -np.ndarray.sum(make_onehot(d[0],
                               self.out_vocab_size) * np.log(Y[-1]))
    ...
    def compute_acc_np(self, x, d):
        ...
        Y = self.predict(x)[0]              # predict output for x
        if np.argmax(Y[-1]) == d[0]:  # if most likely according to prediction (largest in final Y) = singular
            return 1.                        # then return 1, else 0
    ...
    def compare_num_pred(self, x, d):
        ...
        y = self.predict(x)[0]                  # predictions
        d_out = d[0]                            # desired output
        d_infl = d[1]                           # inflected form

        if y[-1][d_out] > y[-1][d_infl]:        # if prediction prob of desired output > infl. form, return 1
            return 1.
        else:
            return 0.
    ...
    def compute_mean_loss(self, X, D):
        ...
        total = 0
        num = 0
        for i in range(len(X)):                         # go through sentences
            total += self.compute_loss(X[i], D[i])      # add to total the loss of each sentence
            num += len(X[i])                            # add to word total
        mean_loss = total / num                         # ML is total loss / number of words
        return mean_loss
...
if __name__ == "__main__":   
    ...
    if mode == "train-lm":
        ...
        nn.train(X_train, D_train, X_dev, D_dev, learning_rate=lr, back_steps=lookback)

        ml = nn.compute_mean_loss(X_test, D_test)
        al = adjust_loss(ml, fraction_lost, q)
        run_loss = ml
        adjusted_loss = al
        ...
        # Question 2b
        # np.save('rnn.U.npy', nn.U)
        # np.savetxt('rnn.U.txt', nn.U)
        # np.save('rnn.W.npy', nn.W)
        # np.savetxt('rnn.W.txt', nn.W)
        # np.save('rnn.V.npy', nn.V)
        # np.savetxt('rnn.V.txt', nn.V)

        # np.save('rnn.deltaU.npy', nn.deltaU)
        # np.savetxt('rnn.deltaU.txt', nn.deltaU)
        # np.save('rnn.deltaW.npy', nn.deltaW)
        # np.savetxt('rnn.deltaW.txt', nn.deltaW)
        # np.save('rnn.deltaV.npy', nn.deltaV)
        # np.savetxt('rnn.deltaV.txt', nn.deltaV)
    ...
    if mode == "train-np":
        ...
        # make the NN
        nn = RNN(vocab_size, hdim, 2)

        # train the NN
        nn.train_np(X_train, D_train, X_dev, D_dev, learning_rate=lr, back_steps=lookback)

        # compute accuracy
        acc = sum([nn.compute_acc_np(X_test[i], D_test[i]) for i in range(len(X_test))]) / len(X_test)
    ...
    if mode == "predict-lm":
        ...
        np_acc = r.compute_acc_lmnp(X_np_dev, D_np_dev) # so not actually training with this dev set. trained with LM dev set.
